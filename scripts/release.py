#!/usr/bin/env python3
"""Release helper for contract-lint (mirrors the contract-ops suite).
...
printed at the end. Publishing happens only when the v* tag is pushed (Trusted Publishing).
"""
...
def read_module_version() -> str:
    text = (ROOT / "contract_lint_cli.py").read_text(encoding="utf-8")
    m = re.search(r'^__version__\s*=\s*"([^"]+)"', text, re.MULTILINE)
    if not m:
        fail("could not find __version__ in contract_lint_cli.py")
...
def main(argv: list[str]) -> int:
    ...
    py = sys.executable
    run(py, "-m", "mypy", "--strict", "contract_lint_cli.py")
    run(py, "-m", "pytest", "-q")
    run(py, "-m", "build")
    ...
    run("git", "-C", str(ROOT), "tag", "-a", tag, "-m", f"contract-lint {version}")
